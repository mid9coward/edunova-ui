...
const ProfileSidebar = ({activeTab, onTabChange}: ProfileSidebarProps) => {
	...
	return (
		<>
			{/* Desktop Sidebar */}
			<Card className="hidden lg:block h-fit pt-0 border-0 shadow-lg bg-gradient-to-br from-card to-muted/30">
				<CardContent className="p-0">
					...
					<div className="p-6 flex flex-col items-center gap-2 justify-center bg-gradient-to-br from-primary/5 to-primary/10 dark:from-primary/10 dark:to-primary/20">
						<div className="relative">
							<div className="h-20 w-20 relative">
								<Avatar className="w-full h-full mx-auto mb-4 ring-4 ring-background/50 shadow-lg">
								...
								{/* Online indicator */}
								<div className="absolute bottom-2 right-0 size-3 bg-primary rounded-full border-2 border-background shadow-sm"></div>
							</div>
						</div>
					...
					<div className="p-3">
							...
							return (
								<Button
									...
									className={`w-full justify-start mb-2 h-12 transition-all duration-200 font-medium ${
										isActive
											? "bg-gradient-to-r from-primary to-primary/90 text-primary-foreground hover:text-primary-foreground shadow-md hover:shadow-lg transform scale-[1.02]"
											: "hover:bg-muted text-muted-foreground hover:text-foreground"
									}`}
									onClick={() => onTabChange(item.id)}
								>
									<Icon
										className={`h-5 w-5 mr-3 ${isActive ? "text-primary-foreground" : ""}`}
									/>
									{item.label}
								</Button>
							);
						...
						<Button
							variant="ghost"
							className="w-full justify-start text-destructive hover:text-destructive hover:bg-destructive/10 h-12 font-medium transition-all duration-200"
					...
					<CardContent className="p-4">
						<div className="flex items-center gap-3">
							<div className="relative">
								...
								{/* Online indicator */}
								<div className="absolute bottom-0 right-0 size-2.5 sm:size-3 bg-primary rounded-full border-2 border-background shadow-sm"></div>
							</div>
							...
							<Button
								variant="ghost"
								size="sm"
								className="text-destructive hover:text-destructive hover:bg-destructive/10 h-8 w-8 sm:h-9 sm:w-9 p-0 flex-shrink-0"
								onClick={handleLogout}
							>
